...
def test_decompose_ed_randomly_and_using_Davenport():
    """Some simple shape and NaN checks."""

    # check that NaNs stay NaNs
    assert np.isnan(decompose_ed_randomly_and_using_Davenport(np.nan)).all()

    # check shapes
    a, fwhm = decompose_ed_randomly_and_using_Davenport(np.logspace(-2,5,100))
    assert a.shape[0] == 100
    assert fwhm.shape[0] == 100
